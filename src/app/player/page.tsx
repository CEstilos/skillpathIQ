...
function PlayerView() {
  ...
  async function toggleDrill(drillId) {
    ...
    if (isCompleted) {
      ...
      const { data } = await supabase
        .from('completions')
        .insert({ drill_id: drillId, player_id: playerId })
        .select()
        .single()
    ...
    <div style={{ minHeight: '100vh', background: '#0E0E0F', fontFamily: 'sans-serif', padding: '24px 16px' }}>
      <div style={{ maxWidth: '440px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#1A1A1C', border: '1px solid #2A2A2D', borderRadius: '16px', padding: '24px', marginBottom: '16px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '13px', color: '#6B6B72' }}>
          ...
          <>
            ...
            {allDone && (
              <div style={{ background: 'rgba(29,184,122,0.1)', border: '1px solid #1DB87A', borderRadius: '12px', padding: '16px', textAlign: 'center', marginBottom: '16px' }}>
                <p style={{ fontSize: '15px', color: '#1DB87A', fontWeight: 600, margin: 0 }}>
                  🔥 All drills done! Coach will see your progress.
                </p>
              </div>
            )}
              ...
              {drills.map(drill => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: isDone ? 'rgba(29,184,122,0.05)' : '#1A1A1C',
                      border: `1px solid ${isDone ? '#1DB87A' : '#2A2A2D'}`,
                      borderRadius: '12px',
                      padding: '16px',
                      display: 'flex',
                      alignItems: 'center',
                    ...
                    <div style={{
                      ...
                      borderRadius: '50%',
                      border: isDone ? 'none' : '2px solid #6B6B72',
                      background: isDone ? '#1DB87A' : 'transparent',
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '15px', fontWeight: 500, color: isDone ? '#6B6B72' : '#ffffff', textDecoration: isDone ? 'line-through' : 'none', transition: 'all 0.15s' }}>
                        {drill.title}
                      </div>
                      {drill.reps && (
                        <div style={{ fontSize: '12px', color: '#6B6B72', marginTop: '3px' }}>{drill.reps}</div>
                      )}
                      {drill.description && (
                        <div style={{ fontSize: '12px', color: '#6B6B72', marginTop: '3px' }}>{drill.description}</div>
                      )}
                    </div>
                  </div>
                )
