...
function NewDrillWeekForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data: week, error: weekError } = await supabase
      .from('drill_weeks')
      .insert({ trainer_id: user.id, group_id: groupId, title, week_start: weekStart })
      .select()
      .single()
      ...
      <div style={{ maxWidth: '640px', margin: '48px auto', padding: '0 16px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ fontSize: '13px', fontWeight: 600, color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Drills ({drills.length}/7)</div>
